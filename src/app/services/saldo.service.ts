...
export class SaldoService {
  ...
  constructor(private http: HttpClient) { }
	...
	public getSaldo(token: string, idUsuario: string, headers: any) {
		return this.http.get('http://66.42.95.58/api/Monedero/Consultar?idUsuario=' + idUsuario,
		{
			headers: headers
		});
	}
	...
	public obtenerTarjetas(token: string, idUsuario: string, headers: any) {
		return this.http.get('http://66.42.95.58/api/Dashboard/Tarjetas?idUsuario=' + idUsuario,
		{
			headers: headers
		});
	}
	...
	public obtenerCuentas(token: string, idUsuario: string, headers: any) {
		return this.http.get('http://66.42.95.58/api/Dashboard/Cuentas?idUsuario=' + idUsuario,
		{
			headers: headers
		});
	}
	...
	public recargarConTarjeta(token: string, idUsuario: string, headers: any) {
		return this.http.post('http://66.42.95.58/api/Monedero/RecargaMonederoTarjeta',
		{
			'idUsuarioReceptor': 0,
			'idMedioPaga': 0,
			'monto': 0,
	...
	public recargarConCuenta(token: string, idUsuario: string, headers: any) {
		return this.http.post('http://66.42.95.58/api/Monedero/RecargaMonederoCuenta',
		{
			'idUsuarioReceptor': 0,
			'idMedioPaga': 0,
			'monto': 0,
